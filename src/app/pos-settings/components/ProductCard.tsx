...
interface ProductCardProps {
  ...
  onToggleVisibility?: (product: Product, isVisible: boolean) => void;
}
...
export function ProductCard({
  ...
  return (
    ...
    >
      {/* Image */}
      <div
        className={`aspect-[16/10] w-full overflow-hidden relative bg-gray-50 ${!product.isVisible ? "grayscale" : ""}`}
      >
        {product.image ? (
          <img
            src={product.image}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <span
            className={`shrink-0 inline-flex items-center text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full ${
        ...
        <div className="space-y-2 mb-3">
          {product.sizes.length > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider w-9 shrink-0">
                Size
              </span>
        ...
        <div className="pt-3 flex items-center justify-between border-t border-gray-100">
          <div className="flex items-center gap-1.5">
            <div
              className={`w-2 h-2 rounded-full ${product.hasRecipe ? "bg-emerald-400" : "bg-gray-300"}`}
            />
            <span
              className={`text-[11px] font-medium ${product.hasRecipe ? "text-emerald-600" : "text-gray-400"}`}
            >
              {product.hasRecipe ? "Recipe Ready" : "No Recipe"}
            </span>
          </div>
          {(() => {
            const uniqueIngredientsCount = Object.values(
              product.recipes || {},
            ).reduce((acc, recipe) => {
            ...
            return (
              uniqueIngredientsCount > 0 && (
                <span className="text-[10px] text-gray-400 font-medium">
                  {uniqueIngredientsCount} ingredient
        ...
        <div
          className="pt-3 mt-3 border-t border-gray-100 flex items-center justify-between"
