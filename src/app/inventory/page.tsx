...
export default function InventoryPage() {
  ...
  const handleSaveReplenishment = async (
    ...
    const { data: txnData, error: txnError } = await supabase
      ...
      .insert({
        ...
        expense_payment_method:
          transactionData.expenseDetails?.paymentMethod ?? null,
    ...
    if (updateError) {
      alert(
        `Stock updated in ledger but failed to update ingredient stock: ${updateError.message}`,
      );
      return;
    }
  ...
  const handleDeleteCategory = async (id: string) => {
    if (items.some((i) => i.categoryId === id)) {
      alert(
        "Cannot delete category: Ingredients are currently assigned to it.",
      );
      return;
    }
    const supabase = createClient();
