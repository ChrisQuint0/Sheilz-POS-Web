...
export function AddTransactionModal({ isOpen, onClose, onSave, currentUser }: AddTransactionModalProps) {
  ...
  const updateItem = (id: string, field: keyof FormOrderItem, value: any) => {
    setItems(prev => prev.map(item => {
      ...
      if (field === 'productName') {
        ...
        const product = PRODUCT_CATALOG.find(p => p.name === updated.productName);
        if (product) {
          const variant = product.variants.find(v => 
             (v.size === updated.size || (v.size === null && updated.size === null)) && 
             (v.temp === updated.temp || (v.temp === null && updated.temp === null))
          );
          if (variant) {
  ...
  const handleSave = () => {
    ...
    const parsedItems: OrderItem[] = items.map(item => {
       ...
       return {
         ...
         temp: item.temp as OrderItem["temp"],
         qty: item.qty
       };
    });
  ...
  const productCategories = Array.from(new Set(PRODUCT_CATALOG.map(p => p.category)));
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Build an order manually. The Order ID and timestamp will be generated automatically.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-3">
               ...
               {items.map((item, index) => {
                  ...
                  const availableSizes = product ? Array.from(new Set(product.variants.map(v => v.size).filter(Boolean))) as string[] : [];
                  
                  const validVariantsForSize = product ? product.variants.filter(v => v.size === item.size) : [];
                  const filteredTemps = Array.from(new Set(validVariantsForSize.map(v => v.temp).filter(Boolean))) as string[];
                  ...
                  return (
                    <div key={item.id} className="flex flex-col gap-3 p-4 border rounded-lg bg-card shadow-sm relative">
                       <div className="flex items-center justify-between">
                          <div className="font-semibold text-sm text-foreground/80">Item {index + 1}</div>
                          <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => setItems(items.filter(i => i.id !== item.id))}>
                       ...
                       <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end">
                              ...
                              <SelectContent>
                                {productCategories.map(category => (
                                   <SelectGroup key={category}>
                                     ...
                                     {PRODUCT_CATALOG.filter(p => p.category === category).map(p => (
                         ...
                         {filteredTemps.length > 0 ? (
                           <div className="sm:col-span-2 space-y-1.5">
                             ...
                             <Select value={item.temp || ""} onValueChange={(v) => updateItem(item.id, 'temp', v)}>
                                <SelectTrigger className="h-9"><SelectValue placeholder="Temp" /></SelectTrigger>
                                <SelectContent>
                                  {filteredTemps.map(t => (
                         ...
                         <div className="sm:col-span-2 space-y-1.5">
                           ...
                           <Input className="h-9" type="number" min="1" value={item.qty} onChange={(e) => {
                              const val = parseInt(e.target.value);
                              if (!isNaN(val) && val > 0) updateItem(item.id, 'qty', val);
                           }} />
                         </div>
                                  ...
                                  <Checkbox 
                                     ...
                                     onCheckedChange={(checked) => {
                                        const newAddons = checked 
                                          ...
                                          : item.selectedAddons.filter(a => a !== addon.name);
                                        updateItem(item.id, 'selectedAddons', newAddons);
                                     }}
                                  />
                                  <Label htmlFor={`${item.id}-${addon.name}`} className="text-xs cursor-pointer font-normal">
        ...
        <DialogFooter className="mt-4 pt-4 border-t">
          ...
          <Button onClick={handleSave} disabled={items.length === 0 || items.some(i => !i.productName)}>
            Save Transaction
          </Button>
        </DialogFooter>
      </DialogContent>
